...
const userInfo = parse<Record<string, string>>(
...
test.skip("Verify login with valid credentials", () => {
  ...
  test("Verify pressing Enter triggers login", async ({
    loginPage,
    dashHomePage,
  }) => {
...
test.skip("Verify that login validation messages are displayed correctly", () => {
  test("User should see error message when username is empty", async ({
    loginPage,
  }) => {
    await loginPage.login("", userInfo[0].Password);
    await loginPage.assertRedBorder(loginPage.passwordInput);
  });
  test("User should see error message when password is empty", async ({
    loginPage,
  }) => {
    await loginPage.login(userInfo[0].UserName, "");
    await loginPage.assertRedBorder(loginPage.usernameInput);
  });
  test("User should see error message with invalid username", async ({
    loginPage,
  }) => {
    await loginPage.login("invalidUser", userInfo[0].Password);
    await loginPage.assertInvalidCredentials();
  });
  test("User should see error message with invalid password", async ({
    loginPage,
  }) => {
